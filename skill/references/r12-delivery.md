...
5. 中英文摘要与关键词齐全;
6. **正文零标记**:全文扫描,不得出现 `[待核实]`/`[待补充]`/`[待确认]` 等任何过程性标记;
7. 待核实数据清单完整(逐条含位置/当前值/替换建议);
8. 系统先行项目:`work/<系统交付目录>/` 在交付清单中。
...
4. **待核实数据清单**:默认假设 + 生成数据 + 需索要材料——逐条列出:位置(章节/表号)、当前值、建议替换方式;用户必须自行核实/替换,论文正文本身保持干净;
